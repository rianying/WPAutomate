...
def extract_segment(inv_number, channel):
    if channel == "panel":
        ...
        if not parts[smr_index + 1].startswith(
            "23"
        ):  # It's a compound segment, add the next part to the segment
...
def clean(datapath):
    ...
    if "SMR_JKT_DIY" in datapath:
        ...
        smr.rename(
            columns={
                "No. Pesanan": "inv_number",
                "Tgl Pesan": "order_time",
                "No. PO": "po_number",
                "No. Pelanggan": "customer_id",
                "Nama Pelanggan": "customer_name",
                "Nama Penjual": "sales_name",
                "Name Syarat Pembayaran": "term_payment",
            },
            inplace=True,
        )
        ...
        panel.rename(
            columns={
                "No Faktur": "inv_number",
                "Administration Time": "order_time",
                "Customer Id": "customer_id",
                "Customer Name": "customer_name",
                "Term Payment": "term_payment",
                "Sales Name": "sales_name",
                "Po Expired": "po_expired",
                "Keterangan": "note",
            },
            inplace=True,
        )
...
def process(data):
    ...
    if df["channel"].iloc[0] == "smr":
        ...
        for segment, startcode in tqdm(
            segments_start_code.items(), desc="Processing segments"
        ):
            try:
...
def generate_query(data, customer_names, po_expire):
    ...
    if data["channel"].iloc[0] == "smr":
        for index, row in tqdm(
            data.iterrows(), total=data.shape[0], desc="Generating queries"
        ):
            ...
            no_SO = (
                ""
                if isinstance(row["inv_number"], float)
                and math.isnan(row["inv_number"])
                else row["inv_number"]
            )
            no_inv = no_SO.replace("SO", "INV")
            no_po = (
                ""
                if isinstance(row["po_number"], float) and math.isnan(row["po_number"])
                else row["po_number"]
            )
            ...
            start_time = (
                pd.to_datetime(order_time) + pd.Timedelta(minutes=fat_random_minute)
            ).strftime("%Y-%m-%d %H:%M:%S")
            finish_time = (
                pd.to_datetime(start_time) + pd.Timedelta(minutes=fat_random_minute)
            ).strftime("%Y-%m-%d %H:%M:%S")
            ...
            validation_value = "('{}','{}','{}','{}','{}')".format(
                no_inv, start_time, finish_time, "Process", ""
            )
            validation_query_values.append(validation_value)

        if new_customer_query_values:
            print("\n\nNew customer found\n\n")
            new_customer_query = "INSERT INTO customer (customer_id, customer_name, customer_address, regency, province) VALUES {}".format(
        ...
        if preorder_query_values:
            ...
            for i in range(0, len(preorder_query_values), batch):
                ...
                preorder_query = "INSERT INTO preorder (inv_number, po_number, channel, customer_id, customer_name, order_time, po_expired, term_payment, sales_name, note) VALUES {}".format(
                    ",".join(batch_value)
                )
            ...
            if validation_query_values:
                ...
                for i in range(0, len(validation_query_values), batch):
                    ...
                    validation_query = "INSERT INTO validation (inv_number, start_check, finish_check, fat_status, note) VALUES {}".format(
                        ",".join(batch_value)
                    )
...
def insert(query):
    connection = pymysql.connect(
        host="192.168.1.219", user="root", password="sarwa", db="osc_clone"
    )
    messages = None
    ...
    try:
        ...
        with connection.cursor() as cursor:
            for i, line in tqdm(
                enumerate(query_lines), total=len(query_lines), desc="Executing queries"
            ):
    ...
    try:
        while True:
            ...
            if smr_exists:
                ...
                print("\nMoving SMR file\n")
                shutil.move(
                    smr_path, smr_path.replace("PANEL&SMR", "PANEL&SMR/ORIGINAL")
                )
            ...
            if panel_exists:
                ...
                print("\nMoving Panel file\n")
                shutil.move(
                    panel_path, panel_path.replace("PANEL&SMR", "PANEL&SMR/ORIGINAL")
                )
