...
def generate_queries(csv_file_path):
    ...
    for _, row in df.iterrows():
        ...
        customer_name = (
            row["customer_name"] if not pd.isna(row["customer_name"]) else ""
        )
        order_time = (
            row["order_time"] if not pd.isna(row["order_time"]) else ""
        )  # Extract order_time from the current row

        fat_random_minutes = random.randint(
            1, 10
        )  # Generate random number of minutes between 1 and 10
        fat_start_time = (
        ...
        value_clause_preorder = (
            f"(\"{no_SO}\", \"{customer_name}\", '{order_time}', '{po_expired}')"
        )
        value_clauses_preorder.append(value_clause_preorder)

        value_clause_order_checking_start = (
            f'("{no_SO}", \'{fat_start_time}\', "{customer_name}")'
        )
        ...
        fat_checking_finish = (
            pd.to_datetime(fat_start_time) + pd.Timedelta(minutes=fat_random_minutes)
        ).strftime("%Y-%m-%d %H:%M:%S")
    ...
    values_str_order_checking_start = ",\n".join(
        value_clauses_order_checking_start
    )  # Join all value clauses for order_checking_start table together, separated by commas
    values_str_order_checking_finish = ",\n".join(
        value_clauses_order_checking_finish
    )  # Join all value clauses for order_checking_finish table together, separated by commas
...
if __name__ == "__main__":
    CheckPO = env.validate[
        "CheckPO"
    ]  # Replace this with the actual path of your CSV file
    ...
    if os.path.exists(CheckPO):
        try:
            ...
            ) = generate_queries(CheckPO)
            queries_combined = (
                f"{queries_order_checking_start}\n\n{queries_order_checking_finish}"
            )
