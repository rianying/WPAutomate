...
def execute_query(full_query):
    ...
    queries = full_query.strip().split(
        "delimiter"
    )  # Splitting full_query into individual queries
    connection = pymysql.connect(
        host="192.168.1.291", user="root", password="sarwa", db="osc_clone"
    )
...
def clean(input_file, output_file):
    ...
    data.rename(
        columns={
            "Tgl Pesan": "order_date",
            "Unnamed: 2": "no_SO",
            "No. Pesanan": "customer_number",
            "Unnamed: 4": "customer_name",
            "Unnamed: 6": "no_PO",
        },
        inplace=True,
    )
...
def process_csv(data):
    ...
    for segment, startcode in segments_startcodes.items():
        try:
            ...
            segment_entry = df.loc[
                matched_rows[0], "no_SO"
            ]  # Use the first matched row as sample
...
def generate_single_query(df, customer_names, po_expire_data):
    ...
    for index, row in df.iterrows():
        ...
        no_PO = (
            ""
            if isinstance(row["no_PO"], float) and math.isnan(row["no_PO"])
            else row["no_PO"]
        )
        no_SO = (
            ""
            if isinstance(row["no_SO"], float) and math.isnan(row["no_SO"])
            else row["no_SO"]
        )
        ...
        if customer_name == "":
            ...
            if customer_name_input not in customer_names.values():
                ...
                customer_address = input(
                    f"Enter customer address for {customer_name_input}: "
                )

                new_customer_values = "('{}', '{}', '{}', '{}', '{}')".format(
                    customer_number,
                    customer_name_input,
                    customer_address,
                    regency,
        ...
        if "SO" in no_SO:
            ...
            fat_start_time = (
                pd.to_datetime(order_time) + pd.Timedelta(minutes=fat_random_minutes)
            ).strftime("%Y-%m-%d %H:%M:%S")
            fat_finish_time = (
                pd.to_datetime(fat_start_time)
                + pd.Timedelta(minutes=fat_random_minutes)
            ).strftime("%Y-%m-%d %H:%M:%S")

            po_expire_value = po_expire_data.get(
                customer_name, 4
            )  # Default to 4 days if not found
            po_expired = order_time + np.timedelta64(po_expire_value, "D")

            pre_order_values = "('{}', '{}', '{}', '{}', '{}','{}','{}','{}')".format(
                no_SO,
                no_PO,
                "panel",
                customer_number,
            ...
            validation_value = "('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')".format(
                no_SO, fat_start_time, fat_finish_time, "Process", str(input("Note:"))
            )
            validation_values.append(validation_value)
        else:
            p_expire_value = po_expire_data.get(
                customer_name, 4
            )  # Default to 4 days if not found
                ...
                try:
                    ...
                    if not processed_data.empty:
                        query = generate_single_query(
                            processed_data, customer_names, po_expire_data
                        )
