...
while True:
    try:
        ...
        if current_last_row > last_row:
            ...
            new_entry_time = None
            new_entry_so = sheet.cell(
                current_last_row, 3
            ).value  # Assuming 'NO SO' is in the third column
            ...
            print(
                "\nTime: {}\nNo new entry found. Last checked row: {} with the value of {} - {}. Order time: {}".format(
                    current_time,
                    last_row,
                    new_entry_segment,
                    new_entry_so,
        ...
        if "RATE_LIMIT_EXCEEDED" in str(api_error):
            current_sleep_interval = exponential_backoff(
                current_sleep_interval, max_sleep_interval, backoff_multiplier
            )
            print(
                f"Time: {datetime.now()}. Error: {api_error}. Sleeping for {current_sleep_interval} seconds."
            )
        else:
